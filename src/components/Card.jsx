      ...
      <div className="col mb-5">
        <div className="card h-100">
          ...
          <div className="card-footer p-4 pt-0 border-top-0 bg-transparent">
            <div className="text-center">
            <button className="btn btn-outline-dark mt-auto" onClick={() => {btnText === "Add to Cart"? `${setBtnText("Remove from Cart")} ${setCount(count + 1)} `: `${setBtnText("Add to Cart")} ${setCount(count - 1)}`;setProduct(data.id);}}>{btnText}</button>
            </div>
          </div>
        </div>
      </div>
